...
func main() {
	...
	for update := range updates {
		...
		if !update.Message.IsCommand() {
			...
			if (isGroup && !strings.HasPrefix(update.Message.Text, "@"+bot.Self.UserName)) {
				log.Printf("Not a command, not a private chat, not a mention, ignoring > [%s] %s", update.Message.From.UserName, update.Message.Text)
				continue
			}
			...
			debouncedEdit := ratelimit.DebounceWithArgs((3 * time.Second), func(text interface{}, messageId interface{}) {
